...
let current_users:string[]=["maryam","mira","mehak","zainab","sara"];
//creating an array of new user name 
let new_users:string[]=["zoya","ahmed","ayesha","sara","maryam"];
//loop through array for checking user name 
new_users.forEach(one_user=>{
    if(current_users.some(current_username=>current_username.toLowerCase()===one_user.toLowerCase())){
        console.log(`${one_user },will need to enter a new username`)
    }
    else{
        console.log(`${one_user},is availabe`)
    }
}
