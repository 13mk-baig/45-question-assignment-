...
let glist: string[]=["aleesha","muzna","afifa","umia"];//creating list fo guest using array
let guestdontcome = glist[1];// here indicatE the index of array to remove thet name
...
glist.forEach(gelist=> console.log(`${glist}"would you like to join me on dinner"`));
...
{
       let removeglist =glist.pop();// make varaible and use pop function  to remove name from the list 
       console.log(`sorry ,${removeglist} i can't invite you to dinner `);
}
glist.forEach(twoguest=> console.log(`${twoguest}"you are still invited"`));//print message to the two person that are still invited 
